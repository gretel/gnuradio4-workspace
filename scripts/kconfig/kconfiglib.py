        ...
        if fn in self.variables:
            ...
            if len(args) == 1:
                ...
                if var._n_expansions:
                    self._parse_error(
                        "Preprocessor variable {} recursively references itself".format(
                            var.name
                        )
                    )
                ...
                self._parse_error(
                    "Preprocessor function {} seems stuck in infinite recursion".format(
                        var.name
                    )
                )
...
class Symbol(object):
    ...
    def _warn_select_unsatisfied_deps(self):
        ...
        for select in split_expr(self.rev_dep, OR):
            ...
            msg += "\n - {}, with value {}, direct dependencies {} (value: {})".format(
