...
RUN apt-get update && apt-get install -y --no-install-recommends \
    cmake ninja-build ccache g++-14 make pkgconf git ca-certificates python3 libcpp-httplib-dev libsoundio-dev \
    && rm -rf /var/lib/apt/lists/*
COPY . /src/gnuradio4-workspace
WORKDIR /src/gnuradio4-workspace
ENV CXX=g++-14
RUN cmake --preset linux -DBUILD_CONFIG=full && cmake --build build/dev \
