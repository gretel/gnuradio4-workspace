...
| 14 | `main` (this PR) | **Deterministic builds**: pin git abbrev length (`--short=12`, Kconfig `GIT_ABBREV_LEN`), `SOURCE_DATE_EPOCH`-aware SBOM timestamps, `CMAKE_BUILD_RPATH_USE_ORIGIN=ON`. **Portability**: `CMAKE_CXX_EXTENSIONS=OFF` by default (Kconfig `CXX_EXTENSIONS` escape), policy floor `3.27...4.3`, CMakePresets v10 with `$comment`. **Developer tooling**: Diagnostics menu (ENABLE_DIAGNOSTICS, CMake ≥4.4), Instrumentation menu (ENABLE_INSTRUMENTATION, CMake ≥4.3), `dev` preset. Fixes: dlfcn-win32 stub for non-Windows, workspace `INTERNAL_ENABLE_BLOCK_PLUGINS` define, `$<INSTALL_PREFIX>` → `${CMAKE_INSTALL_PREFIX}`. | 2 |
